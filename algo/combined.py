...
def wordchangeprob(orig,updated,confusion_set):
    ...
    if len(orig)==len(updated):
        for i in range(0,len(orig)):
            #compute dynamic alpha for 2 cases
            if i==0:
                alpha_d = alpha + (alpha * beta *((10**unigram[vocabtrans(orig[i])])-0.5))
                #print "adding "+str((((10**unigram[vocabtrans(orig[i])])-0.5)))
            else:
                if bigram.has_key((vocabtrans(orig[i-1]),vocabtrans(orig[i]))):
                    alpha_d = alpha + (alpha * beta * ((10**bigram[(vocabtrans(orig[i-1]),vocabtrans(orig[i]))])-0.5))
                    #print "adding"+str((alpha * beta * ((10**bigram[(vocabtrans(orig[i-1]),vocabtrans(orig[i]))])-0.5)))
                else:
                    b_gram=unigram[vocabtrans(orig[i])]+bo_wt_1[vocabtrans(orig[i-1])]
                    alpha_d = alpha + (alpha * beta * ((10 ** b_gram) - 0.5))
                    #print "adding"+str((alpha * beta * ((10 ** b_gram) - 0.5)))
            if alpha_d > 1:
                alpha_d=1
            #print "dynamic alpha :  "+str(alpha_d)
            if orig[i]==updated[i]:
                ...
                prob=prob+math.log10((1-alpha_d)*fr)
        return prob
    else:
...
def printananlysis(testfile,salpha,sbeta):
    ...
    result=open("combined"+testfile.split("/")[-1].split(".")[0]+"_"+str(beta)+".txt",'w+')
...
printananlysis("../corpus_handler/test20_10.txt",0.9,0.10)
printananlysis("../corpus_handler/test20_10.txt",0.9,0.20)
