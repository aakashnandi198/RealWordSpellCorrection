...
loadvocab("/home/aakash/Desktop/nlp_project/corpus_handler/train.vocab")
...
#load unigrams
loadngram("/home/aakash/Desktop/nlp_project/corpus_handler/train.arpa",1)
#load bigrams
loadngram("/home/aakash/Desktop/nlp_project/corpus_handler/train.arpa",2)
#load trigrams
loadngram("/home/aakash/Desktop/nlp_project/corpus_handler/train.arpa",3)
...
def wordchangeprob(orig,updated):
    prob=0
    if len(orig)==len(updated):
        for i in range(0,len(orig)):
            if orig[i]==updated[i]:
                ...
                noofcandidates=len(candidates(orig[i]))
                prob=prob+math.log10((1-alpha)/noofcandidates)
        return prob
    else:
...
def getbestfitsentence(sent):
    ...
    for i in range(0,len(sent)):
        ...
        for j in candidates(sent[i]):
            tmp_lst=sent[:]
            tmp_lst[i]=j
            tmp_prob=getsentenceprob(tmp_lst)+wordchangeprob(sent,tmp_lst)
...
def printananlysis(testfile,salpha):
    alpha=salpha
    truth=[]
    truth_v= [line.lstrip().split(" ") for line in open("/home/aakash/Desktop/nlp_project/corpus_handler/test.txt",'r')]
    ...
    result=open(testfile.split("/")[-1]+"_"+str(alpha)+".txt",'w+')
...
printananlysis("/home/aakash/Desktop/nlp_project/corpus_handler/test20_10.txt",0.9)
